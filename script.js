...
const numberInput = document.getElementById('qualityInput');
...
/* ---------- Quality Sync (Debounced) ---------- */
range.oninput = numberInput.oninput = () => {
  range.value = numberInput.value;
  qval.textContent = range.value + '%';
...
async function processAll(token) {
  ...
  for (const [folder, files] of Object.entries(folderMap)) {
    ...
    for (const file of files) {
      ...
      outputFiles.push({ folder, name, blob });
